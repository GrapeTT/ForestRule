...
CDeer::CDeer(int key, CAnimal *animal)//key控制生成类型
{
	if (key == 1)
	{
		type = 3;
...
void CDeer::freeMove(CAnimal *animal)//自由移动:0向上 1斜右上 2向右 3斜右下 4向下 5斜左下 6向左 7斜左上
{
...
void CDeer::toDrinking(CAnimal *animal)//去饮水
{
	if (hideIndex == 0)
		return;
...
void CDeer::backHome(CAnimal *animal)//返家
{
...
int CDeer::searchEnemy(CAnimal *animal)//搜索敌人
{
	if (hideIndex == 0)
		return -1;

	if (pos_x < 9 && pos_y > 50)//在家里，不用搜索敌人
		return -1;
...
void CDeer::hideEnemy(CAnimal *animal)//躲避敌人
{
	if (hideIndex == -1)
		return;
...
int CDeer::feed(CAnimal *animal)//捕食
{
...
void CDeer::saveData(CAnimal *animal)//保存鹿信息至文件
{
	animal->fDeer<< setw(3) << number  << "号 ";
	if (type == 3)
		animal->fDeer << "幼鹿" << ":" << endl;
	else
		animal->fDeer << "成年鹿" << ":" << endl;
	if (sex == 0)
		animal->fDeer << "          年龄:" << setw(3) << m_age << "      " << "性别:" << "雌" << endl;
	else
		animal->fDeer << "          年龄:" << setw(3) << m_age << "      " << "性别:" << "雄" << endl;
	animal->fDeer << "          能量:" << setw(3) << energy << "      " << "体力:" << physicalStrength << endl;
	animal->fDeer << "          位置:(" << setw(2) << pos_x << "," << setw(2) << pos_y << ")  " << "抑育指数:" << fertilityIndex << endl;
	animal->fDeer << endl;
}
