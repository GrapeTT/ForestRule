...
CWolf::CWolf(int key, CAnimal *animal)//key控制生成类型
{
	if (key == 1)
	{
		type = 5;
...
void CWolf::freeMove(CAnimal *animal)//自由移动:0向上 1斜右上 2向右 3斜右下 4向下 5斜左下 6向左 7斜左上
{
...
void CWolf::toDrinking(CAnimal *animal)//去饮水
{
	if (feedGoal != -1)
		return;
...
void CWolf::backHome(CAnimal *animal)//返家
{
...
int CWolf::searchPrey(CAnimal *animal)//寻找猎物
{	
	if (ifBack == 1)
		return -1;
...
void CWolf::catchPrey(CAnimal *animal)//追赶猎物
{
	if (feedGoal == -1)
		return;
...
int CWolf::feed(CAnimal *animal)//捕食
{
	if (feedGoal == -1)
		return -1;
...
void CWolf::saveData(CAnimal *animal)//保存狼信息至文件
{
	animal->fWolf << setw(3) << number << "号 ";
	if (type == 5)
		animal->fWolf << "幼狼" << ":" << endl;
	else
		animal->fWolf << "成年狼" << ":" << endl;
	if (sex == 0)
		animal->fWolf << "          年龄:" << setw(3) << m_age << "      " << "性别:" << "雌" << endl;
	else
		animal->fWolf << "          年龄:" << setw(3) << m_age << "      " << "性别:" << "雄" << endl;
	animal->fWolf << "          能量:" << setw(3) << energy << "      " << "体力:" << physicalStrength << endl;
	animal->fWolf << "          位置:(" << setw(2) << pos_x << "," << setw(2) << pos_y << ")  " << "抑育指数:" << fertilityIndex << endl;
	animal->fWolf << endl;
}
